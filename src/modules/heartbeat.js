...
export let server = (server, settings) => {
    ...
    server.heartbeat = async () => {
        if (settings.public === true) {
            const body = await fetch(`https://www.classicube.net/heartbeat.jsp?port=${settings.port}&max=${settings['max-players']}&name=${settings['name']}&public=true&version=7&salt=${server.salt}&software=MCC.js%20${require('../../package').version}&users=${server['online_players']}`)
                .then(res => res.json())
                .catch(() => {})
